...
public class YiTiaoLong implements ITingExtension{
	...
	public boolean check(GameSeat gameSeat ,int pai){
		...
		logger.info("YiTiaoLong  用户名{}   牌 {}",gameSeat.getTableSeat().getUserName() , gameSeat.getHoldsPai());
		List<Integer> holds = gameSeat.getHoldsPai();
		...
		if(pai != -1){
			...
			gameSeat.removeHolds(holds.indexOf(pai));
		}else{
			gameSeat.removeHolds(holds.indexOf(endPai));
			gameSeat.addholds(endPai);
		}
